# This file is taken from https://ja-che.github.io/hidimstat/auto_examples/plot_fmri_data_example.html#sphx-glr-auto-examples-plot-fmri-data-example-py
...
import sys
if len(sys.argv) != 4:
    print("Usage: python encludl.py <class1> <class2> <n_threads>")
    sys.exit(1)
...
# np.save(algo+'_final_score.npy', np.array(final_score))
...
def plot_map(pval, one_minus_pval, zscore_threshold, title=None,
             cut_coords=[-25, -40, -5], masker=masker, bg_img=data.bg_img, output_file='default'):
...
plot_map(pval_ecdl, one_minus_pval_ecdl, zscore_threshold_clust, output_file=f'./{algo}_{class1}_{class2}_fmri.png')
