...
def get_score_and_beta(algorithm, random_state, X, y, maxiter): 

    X_train, X_valid, y_train, y_valid = train_test_split(X, y, random_state=random_state, test_size=0.20)
    ...
    if algorithm == 'iht':
        finminmse = np.infty
        for (k, eta) in itertools.product([150], [1/np.linalg.norm(X_train_c, ord='nuc')**2]): 
            max_iter = maxiter
            ...
            for it in range(max_iter):
                if sparse.issparse(X_train_c):
                    ...
                    loss = np.mean((X_train_c @ beta - y_train_c)**2)
                if sparse.issparse(X_train_c):
                    beta -= eta * X_train_c.T * (X_train_c * beta - y_train_c)
                else:
                    beta -= eta * X_train_c.T @ (X_train_c @ beta - y_train_c)
    ...
    if algorithm == 'ksn':
        minmse = np.infty
        for (lam, k, L) in itertools.product([.1, .5, .7, .9, .95, .99, 1], [150], [np.linalg.norm(X_train_c, ord='nuc')**2]):
            max_iter = maxiter
            beta = np.zeros(X_train_c.shape[1])
            theta = 1.
    ...
    if algorithm == 'irksn':
        minmse = np.infty
        for (alpha, k) in itertools.product([0.001], [150]):
            max_iter = maxiter
            beta = np.zeros(X_train_c.shape[1])
            beta_sum = beta + 0.
            beta_avg = beta + 0.
    ...
    if algorithm == 'irosr':
        minmse = np.infty
        for (eta, alpha) in itertools.product([0.001, 0.01, 0.1], [0.001, 0.01, 0.1]):
            max_iter = maxiter
            min_val_loss = np.infty
            curr_loss = np.infty
    ...
    if algorithm == 'srdi':
        minmse = np.infty
        for (kappa, alpha) in itertools.product([0.001, 0.01, 0.1], [0.001, 0.01, 0.1]):
            max_iter = maxiter
            min_val_loss = np.infty
            curr_loss = np.infty
    ...
    final_mse = get_mse(minbeta, X_valid, y_valid, scl, y_offset)
    print('Done.')
    return minbeta, final_mse
